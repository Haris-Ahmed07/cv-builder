...
describe('Resume Routes', () => {
  ...
  test('GET / returns existing resume', async () => {
    Resume.findOne.mockResolvedValue({ _id: 'resume123', user: 'user123', personalInfo: { name: 'Test' } })
    const res = await request(app).get('/api/resume').set('Authorization', token)
  ...
  test('POST / creates resume if not exists', async () => {
    ...
    Resume.findOne.mockResolvedValue(null)
    Resume.create.mockResolvedValue({ ...data, _id: 'new123', user: 'user123' })

    const res = await request(app).post('/api/resume').set('Authorization', token).send(data)
  ...
  test('POST / updates existing resume', async () => {
    const updated = { personalInfo: { name: 'Updated' }, summary: 'Updated summary' }
    const mockResume = { ...updated, _id: 'resume123', user: 'user123', save: jest.fn().mockResolvedValue(true) }

    Resume.findOne.mockResolvedValue(mockResume)

    const res = await request(app).post('/api/resume').set('Authorization', token).send(updated)
  ...
  test('DELETE / deletes resume if exists', async () => {
    ...
    Resume.findOne.mockResolvedValue({ _id: 'resume123', user: 'user123', deleteOne: mockDelete })

    const res = await request(app).delete('/api/resume').set('Authorization', token)
