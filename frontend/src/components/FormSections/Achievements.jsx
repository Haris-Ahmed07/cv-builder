...
const Achievements = () => {
  // Access achievements state and mutation functions from the store
  const { achievements, addAchievement, removeAchievement } = useCVStore()

  // Local state for input value and character counter
  const [input, setInput] = useState('')
  const [charCount, setCharCount] = useState(0)
  const maxChars = 200 // Max allowed characters per achievement

  // Update character counter whenever input changes
  useEffect(() => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    if (!input.trim() || input.length > maxChars) return // Prevent empty or too long input
    addAchievement(input.trim()) // Add trimmed input to state
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-md mb-6">
      {/* Input form for adding new achievements */}
      ...
      {/* Render list of added achievements */}
